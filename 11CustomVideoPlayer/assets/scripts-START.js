/* Get our elements */
const player = document.querySelector('.player');             // overall player element
const video = player.querySelector('.viewer');                // video element
const progress = player.querySelector('.progress');           // progress bar element
const progressBar = player.querySelector('.progress__filled');

const toggle = player.querySelector('.toggle');
const skipButtons = player.querySelectorAll('[data-skip]');
const ranges = player.querySelectorAll('.player__slider');

// const fullScreen = player.querySelector('.fullscreen');
const expandButton = player.querySelector('.expand');

/* build out functions */
function togglePlay(){
  // if(video.paused){
  //   video.play();
  // } else {
  //   video.pause();
  // }
  // ternary operator of the above if/else statement
  const method = video.paused ? 'play' : 'pause';
  video[method]();
  console.log(video[method]);
}

function updateButton(){
  const icon = this.paused ? '►' : '❚ ❚';
  toggle.textContent = icon;
  // console.log('Update the button');
}

let isFullScreen = false;

function updateFullScreenButton(){
  console.log(this.paused);
  const expandicon = this.paused ? '[ ]' : '❚ ❚';
  expandButton.textContent = expandicon;
}

// Toggle the fullscreen class on the video element
function updateFullScreenClasses(){
  player.classList.toggle('fullscreen');
  isFullScreen = !isFullScreen;
}

// Toggle in and out of fullscreen mode
function toggleFullScreen(event){
  console.log(this);
  if(isFullScreen){
    if(document.exitFullscreen){
      document.exitFullscreen();
    } else if (document.msExitFullScreen){
      document.msExitFullScreen();
    }  else {
      console.error('Unable to find a fullscreen exit method.');
    }
    console.log('removing fullscreen class');
  } else {
    if(player.requestFullscreen) {
      player.requestFullscreen(); // standard
    } else if (player.msRequestFullscreen) {
      player.msRequestFullscreen();
    } else {
      console.error('Unable to find a fullscreen request method');
    }
  }
}

function skip(){
  // console.log('Skipping');
  // console.log(this.dataset.skip);
  // helps to understand how much so we actually want to skip
  // add to the current video time the skip amount
  video.currentTime += parseFloat(this.dataset.skip);
}

function handleRangeUpdate(){
  // console.log(this.name);
  // console.log(this.value);
  video[this.name] = this.value;
}

// Automatically update the progress bar as the video plays
function handleProgress(){
  const percent = (video.currentTime / video.duration) * 100;
  progressBar.style.flexBasis = `${percent}%`;
}

// Handle the scrubbing feature
function scrub(event){
  const scrubTime = (event.offsetX / progress.offsetWidth) * video.duration;
  video.currentTime = scrubTime;
  // console.log(event);
}

/* Hook up the event listeners */
video.addEventListener('click', togglePlay);
video.addEventListener('play', updateButton);
video.addEventListener('pause', updateButton);
video.addEventListener('timeupdate', handleProgress);

toggle.addEventListener('click', togglePlay);
skipButtons.forEach(button => button.addEventListener('click', skip));

ranges.forEach(range => range.addEventListener('change', handleRangeUpdate));
ranges.forEach(range => range.addEventListener('mousemove', handleRangeUpdate));

let mousedown = false;
progress.addEventListener('click', scrub);
progress.addEventListener('mousemove', (event) => mousedown && scrub(event));
progress.addEventListener('mousedown', () => mousedown = true);
progress.addEventListener('mouseup', () => mousedown = false);

expandButton.addEventListener('click', updateFullScreenClasses);
// expandButton.addEventListener('click', updateFullScreenButton);
expandButton.addEventListener('click', toggleFullScreen);
